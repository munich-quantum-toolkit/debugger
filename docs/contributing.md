...
[pull requests](https://github.com/munich-quantum-toolkit/debugger/pulls).
...
- 🐛 Report bugs:
  ...
  <https://github.com/munich-quantum-toolkit/debugger/issues>.
...
you can pass {code}`-DBUILD_MQT_DEBUGGER_TESTS=OFF` to the CMake
configure step.
:::
...
pass {code}`-DBUILD_MQT_DEBUGGER_BINDINGS=ON` to the CMake configure
step.
...
The Python package itself lives in the {code}`python/mqt/debugger`
directory.
...
{code}`python/mqt/debugger` directory.
...
changes for MQT Debugger, you can remove the {code}`minor` or {code}`major`
label from the PR.
