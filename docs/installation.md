...
MQT Debugger is primarily developed as a C++20 library with Python bindings. The
Python package is available on [PyPI](https://pypi.org/project/mqt.debugger/)
...
```console
python -m pip install mqt.debugger
```
...
```console
python -c "import mqt.debugger; print(mqt.debugger.__version__)"
```
...
```console
pip install mqt.debugger --no-binary mqt.debugger
```
...
To use the MQT Debugger Python package in your project, add it as a dependency
...
- add it as a [{code}`git` submodule][git-submodule] and build it as part of
  your project, or
- install MQT Debugger on your system and use CMake's {code}`find_package()`
...
of MQT Debugger and only downloads the library if it is not available on the
...
Set up a reproducible development environment for MQT Debugger. This is the
